module.exports = function ($scope, $log, $uibModal, ClientAPIService) {
  ...
  var list = function (page) {
    $scope.loadList = true;
    ClientAPIService.getList('fale_conosco', page)
  ...
  $scope.getAssuntos = function () {
    ClientAPIService.getLoad('fale_conosco/assuntos')
  ...
  $scope.delete = function (key, entity) {
    var modulo = 'fale_conosco/delete/' + entity.id;
  ...
  $scope.deleteSelected = function () {
    ...
    modalInstance.result.then(function () {
      ...
      if (selecteds.length > 0) {
        ClientAPIService.getDelete('fale_conosco/delete', selecteds).success(function (data, status) {
          $scope.itemsSelectedAll = false;
          $scope.message = data.data;
  ...
  $scope.save = function (entity) {
    ...
    if (entity.id) {
      ClientAPIService.getPut('fale_conosco/atualizar/' + entity.id, entity)
    ...
    ClientAPIService.getPost('fale_conosco/salvar', entity)
      .success(function (data, status) {
