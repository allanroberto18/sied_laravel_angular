module.exports = function ($scope, $log, $uibModal, ClientAPIService) {
  ...
  var list = function (page) {
    $scope.loadList = true;
    ClientAPIService.getList('pagina/' + $scope.pagina + '/video', page)
  ...
  $scope.delete = function (key, entity) {
    var modulo = 'pagina/' + $scope.pagina + '/video/delete/' + entity.id;
  ...
  $scope.deleteSelected = function () {
    ...
    modalInstance.result.then(function () {
      ...
      if (selecteds.length > 0) {
        ClientAPIService.getDelete('pagina/' + $scope.pagina + '/video/delete', selecteds).success(function (data, status) {
          $scope.itemsSelectedAll = false;
          $scope.message = data.data;
  ...
  $scope.save = function (entity) {
    ...
    if (entity.id) {
      ClientAPIService.getPut('pagina/' + $scope.pagina + '/video/atualizar/' + entity.id, entity)
    ...
    ClientAPIService.getPost('pagina/' + $scope.pagina + '/video/salvar', entity)
      .success(function (data, status) {
