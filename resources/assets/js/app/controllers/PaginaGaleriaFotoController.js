module.exports = function ($scope, $log, $uibModal, ClientAPIService) {
  ...
  var list = function (page) {
    $scope.loadList = true;
    ClientAPIService.getList('galeria/' + $scope.galeria + '/foto', page)
  ...
  $scope.delete = function (key, entity) {
    var modulo = 'galeria/' + $scope.galeria + '/foto/delete/' + entity.id;
  ...
  $scope.deleteSelected = function () {
    ...
    modalInstance.result.then(function () {
      ...
      if (selecteds.length > 0) {
        ClientAPIService.getDelete('galeria/' + $scope.galeria + '/foto/delete', selecteds).success(function (data, status) {
          $scope.itemsSelectedAll = false;
          $scope.message = data.data;
  ...
  $scope.save = function (entity) {
    ...
    if (entity.id) {
      ClientAPIService.getPut('galeria/' + $scope.galeria + '/foto/atualizar/' + entity.id, entity)
    ...
    ClientAPIService.getPost('galeria/' + $scope.galeria + '/foto/salvar', entity)
      .success(function (data, status) {
